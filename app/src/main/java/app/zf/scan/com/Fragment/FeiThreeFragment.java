...
public class FeiThreeFragment extends BaseFragment implements XListView.IXListViewListener {
    @InjectView(R.id.listview_feithree)
    XListView listviewFeithree;
                    ...
                    if (c.equals("0")) {
                        ...
                        if (mapdata.size() > 0) {
                            if (PageIndexAdd >= recommond) {
                                listviewFeithree.setLoadALL(recommond);
                            }
                            listdadta = (List<Map<String, Object>>) mapdata.get("content");
                            adapter = new FeiMaAllAdapter(listdadta, getActivity());
                            listviewFeithree.setAdapter(adapter);
                            listviewFeithree.stopLoadMore();
                            listviewFeithree.stopRefresh();
                        }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_fei_three, container, false);
        ButterKnife.inject(this, view);
        listviewFeithree.setPullRefreshEnable(true);
        listviewFeithree.setPullLoadEnable(true);
        listviewFeithree.setXListViewListener(this);
