...
public class FeiOneFragment extends BaseFragment implements XListView.IXListViewListener {
    @InjectView(R.id.listview_feiall)
    XListView listviewFeiall;
                    ...
                    if (c.equals("0")) {
                        ...
                        if (mapdata.size() > 0) {
                            if (PageIndexAdd>=recommond){
                                listviewFeiall.setLoadALL(recommond);
                            }
                            listdadta = (List<Map<String, Object>>) mapdata.get("content");
                            adapter = new FeiMaAllAdapter(listdadta, getActivity());
                            listviewFeiall.setAdapter(adapter);
                            listviewFeiall.setVisibility(View.VISIBLE);
                            listviewFeiall.stopLoadMore();
                            listviewFeiall.stopRefresh();
                            dialog.hide();
                        }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_fei_one, container, false);
        ButterKnife.inject(this, view);
        dialog = MyDialog.showDialog(getActivity());
        dialog.show();
        listviewFeiall.setPullRefreshEnable(true);
        listviewFeiall.setPullLoadEnable(true);
        listviewFeiall.setXListViewListener(this);
