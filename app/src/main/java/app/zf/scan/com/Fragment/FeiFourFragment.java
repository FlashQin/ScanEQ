...
public class FeiFourFragment extends BaseFragment implements XListView.IXListViewListener {
    @InjectView(R.id.listview_feifour)
    XListView listviewFeifour;
                    ...
                    if (c.equals("0")) {
                        ...
                        if (mapdata.size() > 0) {
                            if (PageIndexAdd>=recommond){
                                listviewFeifour.setLoadALL(recommond);
                            }
                            listdadta = (List<Map<String, Object>>) mapdata.get("content");
                            adapter = new FeiMaAllAdapter(listdadta, getActivity());
                            listviewFeifour.setAdapter(adapter);
                            listviewFeifour.stopLoadMore();
                            listviewFeifour.stopRefresh();
                        }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_fei_four, container, false);
        ButterKnife.inject(this, view);
        listviewFeifour.setPullRefreshEnable(true);
        listviewFeifour.setPullLoadEnable(true);
        listviewFeifour.setXListViewListener(this);
